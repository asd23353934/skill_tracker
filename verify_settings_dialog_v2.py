...
涵蓋：
- 各欄位初值正確顯示（含快捷鍵限定目標標籤）
- 確認按鈕構出 result dict（12 鍵）並呼叫 app.apply_settings 一次
...
def _build_app():
    ...
    # 完成 / 提前提示音啟用旗標（dialog 以「靜音 = not enable」反向呈現）
    app.enable_end_sound = True
    app.enable_alert_sound = True
    app.alert_before_seconds = 5
    app.window_size = 64  # 中
    app.global_sound = "ding.wav"
    app.global_alert_sound = ""
    app.sound_volume = 80
    # 快捷鍵限定：給真實 str / bool，否則 MagicMock 子物件會落入
    # QLabel(self._hotkey_target_label or ...) 觸發 TypeError
    app.hotkey_app_filter_enabled = False
    app.hotkey_app_target_exe = ""
    app.hotkey_app_target_label = ""
...
def test_initial_values():
    ...
    # enable_*_sound = True → 靜音 checkbox 不勾
    check("end mute checkbox", dlg.end_mute_cb.isChecked(), False)
    check("alert mute checkbox", dlg.alert_mute_cb.isChecked(), False)
    ...
    # 快捷鍵限定：無目標 → 標籤顯示「未選擇」、啟用 checkbox 不可勾
    check("hotkey target label", dlg.hotkey_target_lbl.text(), "未選擇")
    check("hotkey filter cb disabled", dlg.hotkey_filter_cb.isEnabled(), False)
...
def test_confirm_calls_apply_settings():
    ...
    dlg.end_mute_cb.setChecked(True)   # 靜音 → enable_end_sound 應為 False
    ...
    check("result has 12 keys", len(result), 12)
    check("result.x", result["x"], 300)
    check("result.y", result["y"], 250)
    check("result.enable_end_sound (靜音→False)", result["enable_end_sound"], False)
    check("result.enable_alert_sound", result["enable_alert_sound"], True)
    ...
    check("result.hotkey_app_filter_enabled", result["hotkey_app_filter_enabled"], False)
    check("result.hotkey_app_target_exe", result["hotkey_app_target_exe"], "")
    check("result.hotkey_app_target_label", result["hotkey_app_target_label"], "")
