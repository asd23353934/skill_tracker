...
v4.12.0 (2026-08-31)
-------------------
✨ 技能小窗新增重置鈕（關閉鈕左側，與關閉鈕同尺寸、hover 青色以資區別）
  - 進行中的倒數直接歸零，不播完成音、不觸發提前提示
  - 循環技能歸零後停住不跑下一輪，停在 0 待機，可再按快捷鍵重新啟動
  - 常駐 / 怪物正數視窗歸零後回到待機；一般技能比照自然結束自動關閉
  - 兩顆鈕都有 tooltip（重置 / 關閉），滑鼠離開小窗自動清掉 hover 高亮
✨ 循環技能改為與常駐一樣常駐顯示
  - 勾選循環當下就建立 0 待機小窗，不必等按鍵觸發才看得到
  - 開機還原、切換配置、套用設定、技能詳情對話框四條路徑一併涵蓋
🐛 切到遊戲 / 其他置頂視窗後，技能小窗不再被壓在底層
  - WS_EX_TOPMOST 只保證壓過非置頂視窗；同樣置頂的視窗一旦後取得前景就會排到
    小窗前面，而待機中的常駐 / 循環小窗沒有計時 timer，過去完全沒有機會重申
  - 改為監看前景視窗變化，前景真的換了才重申 z-order。沒有小窗時連前景都不查，
    閒置 20 秒量不到 CPU 佔用；不會有持續 SetWindowPos 造成的閃爍
  - 獨佔全螢幕的遊戲仍無法覆蓋（DWM 之外的路徑），需改用無邊框視窗模式
🐛 修正「全部常駐 / 全部循環」批次切換不會建立或收掉小窗
  - 原本拿已經更新過的狀態去比對「前後是否改變」，判斷恆為否，兩個分支都不執行
  - 改為直接依當下的常駐 / 循環旗標決定，只要其一為開就保留待機小窗
🎯 海盜新增「海鷗特戰隊」5s（Gaviota，神槍手三轉召喚技）
✅ 250 pytest 全綠（新增 7 項置頂 watcher 測試）＋ 全部 verify 腳本通過

v4.11.0 (2026-07-27)
-------------------
🐛 修復「更新後設定被重置成預設值」
  - 起因：使用者資料（config_user.json / profiles / 練功存檔）過去寫在 PyInstaller
    的 _internal/ 目錄裡，那正是更新解壓時會被 ZIP 逐檔覆寫的位置。開發機在 dist/
    跑過 exe 就會生成一份全預設值的 config_user.json，跟著被打包進 release ZIP，
    使用者一更新就被蓋掉（小窗大小、音效、快捷鍵配置全回預設）
  - 使用者資料改放執行檔同層目錄，更新解壓再也碰不到
  - 升級時自動把舊位置（_internal/）的既有資料接過來；新位置已有資料則絕不覆蓋
  - 打包流程新增雙重防線：壓縮時排除所有使用者資料，壓完回讀 ZIP 二次確認，
    驗出使用者資料就刪檔並中止發布
🔧 發布流程整併為單一入口 python release.py
  - 事故一半的成因是流程分散在多個腳本、沒人說得清該跑哪幾個、順序為何
  - 前置檢查 → strip → build → restore → zip 一條龍，restore 放在 finally
    保證執行（原本 build 失敗會讓 config.json 留在出廠預設狀態被 commit）
  - 移除 clean_for_release.py / check_release.py：兩者寫入並檢查
    skill_send / skill_receive / skill_permanent 等現行格式早已不存在的欄位，
    跑下去會毀掉開發機的 config.json 與 profiles，且檢查永遠是紅燈
  - 使用者資料清單收斂到 helpers.py 單一來源，打包排除與遷移不再各存一份
  - 修正 zip_release.py 內失效的 sound_manager import（模組早已搬到
    infrastructure/），原本被 except 吞掉 → 開發機沒有 sounds/ 時發出去的 ZIP
    內建音效是空的；改為直接失敗
✨ 技能倒數小窗置頂強化
  - 每次按鍵觸發、以及提前提示時，重新把小窗拉到所有置頂視窗的最前面
    （原本只在建立時宣告一次置頂，後來出現的其他置頂視窗會蓋在上面）
  - 小窗改為永不取得焦點：拖曳或按關閉鈕時不會再把焦點從遊戲搶走
🐛 修復指令快捷鍵小窗殘留舊按鍵
  - 改綁按鍵後，小窗上舊的那列沒被清除，與新按鍵並存（每刷新一次就多累積一列）
  - 原因是清單列以裸 layout 加入，清空時取不到 widget 可刪，QLabel 脫離 layout
    後仍留在卡片原位顯示；改為每列一個 QWidget 容器，並讓視窗高度跟著縮回
🎨 側邊欄版本號改為淡灰底圓角 chip
  - 原本只是暗色小字，看不出可以點擊；加底色 + hover 變亮後一眼可辨

v4.10.2 (2026-07-24)
-------------------
- 新增「更新日記」功能：側邊欄版本號可點擊，開啟對話框依版本瀏覽完整更新紀錄

v4.10.1 (2026-07-24)
-------------------
- 指令頁新增「重置名稱」/「全部重置按鍵」（確認對話框後才執行）
- 指令頁 needs_name 名稱輸入提示文字依指令區分（僅 /密語 顯示 #代碼提示）
- 快捷鍵限定攔截觸發時顯示提示 toast，取代原本的靜默無反應
- 設定對話框「快捷鍵限定」列補充用途說明文字
- 指令頁分組標頭改用圖示徽章 + 數量標籤，版面更一致
- 修正技能／怪物／指令快捷鍵共同觸發（原本技能命中會讓指令被跳過不觸發）
