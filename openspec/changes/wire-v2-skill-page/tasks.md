...
- [ ] 12.1 `python main.py --v2` 啟動，觀察技能頁首次顯示不卡頓（<300ms）。
- [ ] 12.2 擷取熱鍵、切常駐 / 循環 / 提醒、改冷卻、改提前秒數，切換 profile，回來確認狀態持久化正確。
- [ ] 12.3 開 ⋮ 詳細設定對話框：改結束聲音、試聽、按確認；另一次試聽後按取消確認未寫入。
- [ ] 12.4 V1 / V2 同時切換：V1 改狀態 → 切 V2 頁 → 確認 V2 顯示同步；反之亦然。
