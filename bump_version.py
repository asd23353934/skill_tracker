...
def main():
    ...
    print()
    print("📋 接下來的步驟:")
    print("  1. 發布: python release.py")
    print("     （唯一入口：前置檢查 → strip → build → restore → zip，不要單獨跑個別步驟）")
    print(f"  2. 測試 dist/skill_tracker_v{new_version}.zip")
    print(f"  3. 創建 Git tag: git tag v{new_version}")
    print(f"  4. 推送: git push origin v{new_version}")
    print(f"  5. 在 GitHub 創建 Release，上傳 dist/skill_tracker_v{new_version}.zip")
    print()
